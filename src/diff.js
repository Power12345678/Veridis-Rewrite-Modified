...
function getInsertSource(start, end, aiInsertRanges = [], manualInsertRanges = []) {
    if (rangeOverlapsAny(start, end, manualInsertRanges)) return 'manual';
    if (rangeOverlapsAny(start, end, aiInsertRanges)) return 'ai';
    return 'program';
}

function splitInsertOperationBySource(operation, aiInsertRanges = [], manualInsertRanges = []) {
    const start = Number(operation?.newStart);
    const end = Number(operation?.newEnd);
    if (operation?.type !== 'insert' || !Number.isFinite(start) || !Number.isFinite(end) || end <= start) {
        return [operation];
    }

    const boundaries = new Set([start, end]);
    [...aiInsertRanges, ...manualInsertRanges].forEach((range) => {
        const overlapStart = Math.max(start, Number(range?.start));
        const overlapEnd = Math.min(end, Number(range?.end));
    ...
    for (let index = 0; index < sorted.length - 1; index++) {
        const partStart = sorted[index];
        const partEnd = sorted[index + 1];
        const textStart = partStart - start;
        const textEnd = partEnd - start;
        const text = String(operation.text || '').slice(textStart, textEnd);
        if (!text) continue;
        const source = getInsertSource(partStart, partEnd, aiInsertRanges, manualInsertRanges);
        const previous = parts[parts.length - 1];
        if (previous?.source === source && previous.newEnd === partStart) {
            previous.text += text;
            previous.newEnd = partEnd;
            continue;
        }
        parts.push({
            ...operation,
    ...
    return parts.length > 0 ? parts : [{ ...operation, source: 'program' }];
}

function applyInsertSources(annotatedOperations = [], aiInsertRanges = [], manualInsertRanges = []) {
    return annotatedOperations.flatMap((operation) => {
        if (operation?.type !== 'insert') return operation;
        return splitInsertOperationBySource(operation, aiInsertRanges, manualInsertRanges);
    });
}
...
export function buildDiffResultFromStages(rawText, programText, aiText, manualText) {
    ...
    const programToManualOperations = annotateDiffOperations(getTextDiffOperations(programDisplayText, manualDisplayText));
    const aiToManualOperations = annotateDiffOperations(getTextDiffOperations(aiDisplayText, manualDisplayText));
    const aiInsertRanges = getInsertRanges(programToManualOperations);
    const manualInsertRanges = getInsertRanges(aiToManualOperations);
    const sourceToManualOperations = applyInsertSources(
        annotateDiffOperations(getTextDiffOperations(displayText, manualDisplayText)),
        aiInsertRanges,
        manualInsertRanges
    );
...
function resolveDiffCachePair(msg) {
    ...
    if (diffMeta?.originalMes && diffMeta?.lastCleanedMes && diffMeta.originalMes !== diffMeta.lastCleanedMes) {
        return {
            ...
            aiProgramMes: diffMeta.aiProgramMes || '',
...
export function refreshDiffCacheIfStale(index) {
    ...
    const diffResult = finalSource === 'manual'
        ? buildDiffResultFromStages(sourceMes, aiProgramMes || sourceMes, hasAiTrace ? aiFinalMes : null, cleanedMes)
        : hasAiTrace && aiFinalMes === cleanedMes
